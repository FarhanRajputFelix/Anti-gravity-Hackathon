...
class _OutcomeScreenState extends State<OutcomeScreen>
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: AppTheme.alertYellow.withOpacity(0.3)),
              ),
              child: Text(_showBaseline ? 'Show SAHARA' : 'vs Traditional',
        ...
        child: Column(
          ...
          children: [
            ...
            FadeInUp(
                ...
                child: _buildBeforeAfterComparison()),
            const SizedBox(height: 16),
            FadeInUp(
                delay: const Duration(milliseconds: 150),
                child: _buildCongestionChart()),
            const SizedBox(height: 16),
            FadeInUp(
                delay: const Duration(milliseconds: 200),
                child: _buildImpactMetrics()),
            const SizedBox(height: 16),
            FadeInUp(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
            colors: [AppTheme.navyCard, AppTheme.navyCard.withOpacity(0.8)]),
      ...
      child: Column(children: [
        Row(children: [
          ...
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                const Text('Response Complete',
                    style: TextStyle(
                        color: AppTheme.successGreen,
                        fontSize: 13,
                        fontWeight: FontWeight.w600,
                        letterSpacing: 0.5)),
                Text('${_result['crisis_type']} • ${_result['location']}',
                    style: const TextStyle(
        ...
        Row(children: [
          _buildQuickStat(
              'Execution',
              '${((_result['total_execution_time_ms'] as num?)?.toInt() ?? 3284)}ms',
              AppTheme.glowBlue),
          _buildQuickStat(
              'Confidence',
              '${((((_result['confidence'] as num?)?.toDouble() ?? 0.82) * 100).toStringAsFixed(0))}%',
              AppTheme.successGreen),
          _buildQuickStat(
              'Response',
              '${(_sim['response_time_minutes'] as num?)?.toInt() ?? 8} min',
              ...
              child: _buildStateCard(
                  ...
                  [
                    ...
                    },
                    {
                      'label': 'Responders',
                      'value':
                          '${(_sim['emergency_units_dispatched'] as num?)?.toInt() ?? 6}',
                      'color': AppTheme.electricBlue
                    },
                    {
                      'label': 'Alerts Sent',
                      'value': _formatNum(alertsSent),
                      'color': AppTheme.glowBlue
                    },
                    {
                      'label': 'Helped',
                      'value': _formatNum(popHelped),
                      'color': AppTheme.successGreen
                    },
                  ],
                  AppTheme.successGreen,
  ...
  Widget _buildStateCard(String label, List<Map<String, dynamic>> metrics,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
            ...
            colors: [color.withOpacity(0.08), AppTheme.navyCard]),
  ...
  Widget _buildCongestionChart() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Congestion Reduction',
            style: TextStyle(
                color: AppTheme.textPrimary,
                fontSize: 14,
                fontWeight: FontWeight.w600)),
        ...
        Text(
            '${(before - after).toStringAsFixed(0)} point improvement — ${((before - after) / before * 100).toStringAsFixed(0)}% reduction',
  ...
  Widget _buildImpactMetrics() {
    final metrics = [
      {
        'label': 'Population Helped',
        'value':
            _formatNum((_sim['population_helped'] as num?)?.toInt() ?? 28600),
        'icon': Icons.people,
        'color': AppTheme.electricBlue
      },
      {
        'label': 'Alerts Sent',
        'value': _formatNum((_sim['alerts_sent'] as num?)?.toInt() ?? 34320),
        'icon': Icons.notifications_active,
        'color': AppTheme.alertOrange
      },
      {
        'label': 'Tickets Generated',
        'value': '${((_sim['emergency_tickets'] as List?)?.length ?? 4)}',
        'icon': Icons.confirmation_number,
        'color': AppTheme.cyanAccent
      },
      {
        'label': 'Hospitals Notified',
        'value': '${(_sim['hospitals_notified'] as num?)?.toInt() ?? 2}',
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [
          AppTheme.alertYellow.withOpacity(0.05),
          AppTheme.navyCard
        ]),
        borderRadius: AppTheme.radiusMd,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('SAHARA AI vs Traditional Systems',
            style: TextStyle(
                color: AppTheme.alertYellow,
                fontSize: 14,
                fontWeight: FontWeight.w700)),
        const SizedBox(height: 12),
        _buildCompRow('Detection Time', 'Manual reporting: 25-45 min',
            'SAHARA: < 8 seconds'),
        _buildCompRow('Coordination', '5+ phone calls between depts',
            'Automated: instant notification'),
        _buildCompRow('Accuracy', 'Single source, unverified',
            'Multi-source verified: 82% confidence'),
        _buildCompRow('Coverage', 'City-by-city, no automation',
            'Pakistan-wide: multi-city monitoring'),
        _buildCompRow('Fallback', 'No recovery plan',
            'Automatic API fallback + recovery'),
      ]),
    );
  }
  ...
  Widget _buildTicketList() {
    final tickets = (_sim['emergency_tickets'] as List?)?.cast<String>() ??
        ['DISP-A3F7', 'RTE-B8E2', 'ALT-C5F1', 'MED-D2E3'];
    final icons = [
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Emergency Tickets (${tickets.length})',
          style: const TextStyle(
              color: AppTheme.textPrimary,
              fontSize: 14,
              fontWeight: FontWeight.w600)),
