...
class ExecutionSimulationAgent extends BaseAgent {
  @override
  String get name => 'Execution Simulation Agent';
  @override
  int get index => 5;
  @override
  String get description =>
      'Simulates real-time execution of response plan — dispatch, rerouting, alerts';
  @override
  List<String> get tools => [
        'system_state_snapshot',
        'dispatch_engine',
        'google_maps_rerouting_api',
        'alert_broadcast_engine'
      ];
  ...
  Future<AgentMemory> execute(AgentMemory memory,
    ...
    observations.add(
        'BEFORE: congestion ${memory.congestionBefore}/100, 0 responders, 0 alerts sent.');
    ...
    for (int i = 0;
        i < memory.actionPlan.length && i < ticketPrefixes.length;
        i++) {
      final hex = (rng.nextInt(0xFFFFFF))
          .toRadixString(16)
          .toUpperCase()
          .padLeft(6, '0');
      ...
      memory.executionLogs.add(
          '[$logTime] ✅ $dept: Dispatch order #$ticket issued. ${action['resources'] ?? 'Units deployed'}.');
      observations
          .add('P${i + 1}: $dept — ${action['action']}. Ticket $ticket.');
    }
    reasoning.add(
    ...
    onProgress?.call(
        name, 'Activating route rerouting via Google Maps...', 0.6);
    ...
    reasoning.add(
        'Google Maps rerouting API updated — ${memory.roadsRerouted.length} alternate routes activated, ETA -18 min.');
    ...
    memory.alertsSent = memory.populationAffected > 50000
        ? 34320 + rng.nextInt(5000)
        : 12000 + rng.nextInt(3000);
    ...
    final reductionPct = memory.severity == 'CRITICAL'
        ? 0.63
        : memory.severity == 'HIGH'
            ? 0.5
            : 0.35;
    memory.congestionAfter =
        max(15, (memory.congestionBefore * (1 - reductionPct)).round());
    memory.populationHelped =
        (memory.populationAffected * (0.5 + rng.nextDouble() * 0.3)).round();

    final congDrop = memory.congestionBefore - memory.congestionAfter;
    memory.executionLogs.add(
        '[$timeStr:${(12 * memory.actionPlan.length + 30).toString().padLeft(2, '0')}] 📊 POST-RESPONSE: Congestion ↓${congDrop}pts | ${_formatNum(memory.populationHelped)} residents assisted');

    observations.add(
        'AFTER: congestion ${memory.congestionAfter}/100, ${memory.unitsDispatched} units active, ${_formatNum(memory.alertsSent)} alerts, ${_formatNum(memory.populationHelped)} helped.');
    ...
    memory.agentTraces.add(AgentTrace(
      ...
      decision:
          'All ${memory.actionPlan.length} actions EXECUTED. Congestion: ${memory.congestionBefore}→${memory.congestionAfter}. ${_formatNum(memory.alertsSent)} alerts sent. ${_formatNum(memory.populationHelped)} citizens helped.',
