...
class FallbackRecoveryAgent extends BaseAgent {
  @override
  String get name => 'Fallback & Recovery Agent';
  @override
  int get index => 6;
  @override
  String get description =>
      'Monitors pipeline health, triggers API substitutions, ensures system continuity';
  @override
  List<String> get tools => [
        'pipeline_health_monitor',
        'historical_traffic_db',
        'rule_based_nlp_fallback',
        'resilience_scorer'
      ];
  ...
  Future<AgentMemory> execute(AgentMemory memory,
    ...
    final trafficApiDown =
        memory.crisisType == 'FLOODING' || memory.crisisType == 'EARTHQUAKE';
    final nlpDegraded = memory.crisisType != 'ACCIDENT';
    int fallbacksApplied = 0;

    observations.add(
        'Pipeline health scan: ${trafficApiDown ? "⚠ Traffic API elevated latency" : "✅ Traffic API nominal"}, ${nlpDegraded ? "⚠ NLP API degraded mode" : "✅ NLP API nominal"}.');
    ...
    if (trafficApiDown) {
      ...
      final city = location.contains('Islamabad')
          ? 'Islamabad'
          : location.contains('Karachi')
              ? 'Karachi'
              : 'Lahore';
      ...
      observations.add(
          'Historical pattern for $city: typical congestion = 65/100. Accuracy: ±15%.');
      reasoning.add(
    ...
    if (nlpDegraded) {
      ...
      memory.fallbackStrategies
          .add('Gemini NLP → Rule-based engine (92% accuracy)');
    ...
    memory.systemResilience = fallbacksApplied == 0
        ? 1.0
        : fallbacksApplied == 1
            ? 0.92
            : 0.85;
    ...
    if (fallbacksApplied > 0) {
      reasoning.add(
          'Pipeline continuity maintained through $fallbacksApplied failure scenario${fallbacksApplied > 1 ? 's' : ''}.');
    } else {
      reasoning.add(
          'All systems nominal — no fallbacks required. Pipeline resilience: 100%.');
    }
    observations.add(
        'System resilience: ${(memory.systemResilience * 100).toStringAsFixed(0)}%. All downstream agents unaffected.');
    ...
    avgConfidence = avgConfidence /
        (memory.agentTraces.isEmpty ? 1 : memory.agentTraces.length);
    memory.overallConfidence = avgConfidence * memory.systemResilience;

    sw.stop();
    final execTime = sw.elapsedMilliseconds;
    memory.totalExecutionTimeMs =
        memory.agentTraces.fold<int>(0, (sum, t) => sum + t.executionTimeMs) +
            execTime;
