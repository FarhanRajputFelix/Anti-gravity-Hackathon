...
class SignalIngestionAgent extends BaseAgent {
  @override
  String get name => 'Signal Ingestion Agent';
  @override
  int get index => 1;
  @override
  String get description =>
      'Processes raw crisis signals — language detection, crisis classification, location extraction';
  @override
  ...
  static final _arabicRange = RegExp(r'[\u0600-\u06FF]');
  ...
  Future<AgentMemory> execute(AgentMemory memory,
    ...
    observations.add(
        'Raw signal received (${memory.rawText.length} chars). Normalizing language.');
    observations.add(
        'Language detection: $lang ($urduWordCount Urdu markers, Arabic script: $hasArabicScript).');
    reasoning.add(
        'Language scan: $lang confirmed via keyword markers + script analysis.');
    ...
    memory.keywords =
        _crisisPatterns[bestType]?.where((k) => text.contains(k)).toList() ??
            [];
    observations.add('Keyword cluster matched: ${memory.keywords.join("/")}.');
    reasoning.add(
        'Crisis classifier matched ${memory.crisisType} with ${(memory.signalConfidence * 100).toStringAsFixed(0)}% keyword overlap.');
    ...
    memory.extractedLocation = locDetail != null
        ? '$locDetail, $locCity'
        : memory.locationHint ?? 'Pakistan';
    observations.add(
        'Location entity: ${memory.extractedLocation} extracted from pattern database.');
    reasoning.add(
        'Location entity: ${memory.extractedLocation} extracted via regex + city grid mapping.');
    ...
    reasoning
        .add('Duplicate check passed — cleared for verification pipeline.');
    ...
    memory.agentTraces.add(AgentTrace(
      ...
      decision:
          'Crisis type ${memory.crisisType} detected in ${memory.extractedLocation}. Language: $lang.',
